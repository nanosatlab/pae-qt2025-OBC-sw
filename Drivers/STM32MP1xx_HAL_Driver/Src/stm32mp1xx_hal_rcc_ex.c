	...
	if ((pll2->PLLState) != RCC_PLL_NONE) {
		...
		if (!__IS_PLL2_IN_USE()) /* If not used then */
		{
			if ((pll2->PLLState) == RCC_PLL_ON) {
				...
				__HAL_RCC_PLL2_CONFIG(pll2->PLLM, pll2->PLLN, pll2->PLLP,
						pll2->PLLQ, pll2->PLLR);
				...
				if (pll2->PLLMODE == RCC_PLL_SPREAD_SPECTRUM) {
					...
					__HAL_RCC_PLL2CSGCONFIG(pll2->MOD_PER, pll2->TPDFN_DIS,
							pll2->RPDFN_DIS, pll2->SSCG_MODE, pll2->INC_STEP);
	...
	if ((pll3->PLLState) != RCC_PLL_NONE) {
		...
		if (!__IS_PLL3_IN_USE()) /* If not used then*/
		{
			if ((pll3->PLLState) == RCC_PLL_ON) {
				...
				__HAL_RCC_PLL3_CONFIG(pll3->PLLM, pll3->PLLN, pll3->PLLP,
						pll3->PLLQ, pll3->PLLR);
				...
				if (pll3->PLLMODE == RCC_PLL_SPREAD_SPECTRUM) {
					...
					__HAL_RCC_PLL3CSGCONFIG(pll3->MOD_PER, pll3->TPDFN_DIS,
							pll3->RPDFN_DIS, pll3->SSCG_MODE, pll3->INC_STEP);
	...
	if ((pll4->PLLState) != RCC_PLL_NONE) {
		...
		if ((pll4->PLLState) == RCC_PLL_ON) {
			...
			__HAL_RCC_PLL4CLKOUT_DISABLE(
					RCC_PLL4_DIVP | RCC_PLL4_DIVQ | RCC_PLL4_DIVR);
			/* Disable the main PLL. */
			...
			__HAL_RCC_PLL4_CONFIG(pll4->PLLM, pll4->PLLN, pll4->PLLP,
					pll4->PLLQ, pll4->PLLR);
			...
			if (pll4->PLLMODE == RCC_PLL_SPREAD_SPECTRUM) {
				...
				__HAL_RCC_PLL4CSGCONFIG(pll4->MOD_PER, pll4->TPDFN_DIS,
						pll4->RPDFN_DIS, pll4->SSCG_MODE, pll4->INC_STEP);
...
HAL_StatusTypeDef HAL_RCCEx_PeriphCLKConfig(
	...
	RCC_PERIPHCLK_UART24) {
		/* Check the parameters */
		assert_param(
				IS_RCC_UART24CLKSOURCE(PeriphClkInit->Uart24ClockSelection));
	...
	RCC_PERIPHCLK_UART35) {
		/* Check the parameters */
		assert_param(
				IS_RCC_UART35CLKSOURCE(PeriphClkInit->Uart35ClockSelection));
	...
	RCC_PERIPHCLK_UART78) {
		/* Check the parameters */
		assert_param(
				IS_RCC_UART78CLKSOURCE(PeriphClkInit->Uart78ClockSelection));
	...
	RCC_PERIPHCLK_SDMMC12) {
		/* Check the parameters */
		assert_param(
				IS_RCC_SDMMC12CLKSOURCE(PeriphClkInit->Sdmmc12ClockSelection));
	...
	RCC_PERIPHCLK_SDMMC3) {
		/* Check the parameters */
		assert_param(
				IS_RCC_SDMMC3CLKSOURCE(PeriphClkInit->Sdmmc3ClockSelection));
	...
	RCC_PERIPHCLK_SPDIFRX) {
		/* Check the parameters */
		assert_param(
				IS_RCC_SPDIFRXCLKSOURCE(PeriphClkInit->SpdifrxClockSelection));
	...
	RCC_PERIPHCLK_USBPHY) {
		/* Check the parameters */
		assert_param(
				IS_RCC_USBPHYCLKSOURCE(PeriphClkInit->UsbphyClockSelection));
	...
	RCC_PERIPHCLK_LPTIM45) {
		/* Check the parameters */
		assert_param(
				IS_RCC_LPTIM45CLKSOURCE(PeriphClkInit->Lptim45ClockSelection));
	...
	RCC_PERIPHCLK_LPTIM23) {
		/* Check the parameters */
		assert_param(
				IS_RCC_LPTIM23CLKSOURCE(PeriphClkInit->Lptim23ClockSelection));
	...
	RCC_PERIPHCLK_RTC) {
		...
		if (ret == HAL_OK) {
			...
			if ((RCC->BDCR & RCC_BDCR_RTCSRC)
					!= (PeriphClkInit->RTCClockSelection & RCC_BDCR_RTCSRC)) {
				...
				RESERVED_BDCR_MASK = ~(RCC_BDCR_VSWRST | RCC_BDCR_RTCCKEN
						| RCC_BDCR_RTCSRC |
				...
				WRITE_REG(RCC->BDCR,
						(READ_REG(RCC->BDCR) | (tmpreg & RESERVED_BDCR_MASK)));
...
uint32_t HAL_RCCEx_GetPeriphCLKFreq(uint64_t PeriphClk) {
	...
	/* Check the parameters */
	assert_param(
			IS_RCC_PERIPHCLOCK(PeriphClk) || IS_RCC_PERIPHONECLOCK(PeriphClk));
